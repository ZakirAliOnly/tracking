...
export function TransferFundsDrawer({ open, onClose, accounts, prefilledFromId }: Props) {
  const [state, formAction, isPending] = useActionState<AccountActionState, FormData>(transferFunds, null);
  ...
  useEffect(() => { if (open) setTimeout(() => amountRef.current?.focus(), 300); }, [open]);
