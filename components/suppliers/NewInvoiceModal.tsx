...
export function NewInvoiceDrawer({ open, onClose, suppliers, devices, prefilledSupplierId }: Props) {
  const [state, formAction, isPending] = useActionState<SupplierActionState, FormData>(
  ...
  return (
    <>
      ...
      />
      <div
        className={`fixed right-0 top-0 z-50 flex h-full w-full max-w-[460px] flex-col bg-surface shadow-2xl transition-transform duration-300 ease-out ${open ? "translate-x-0" : "translate-x-full"}`}
      >
        ...
        <form key={formKey} action={formAction} className="flex flex-1 flex-col gap-4 overflow-y-auto px-6 py-6">
